...
export default function PnlPage() {
  ...
  const startOfDayValue = todayPnlData?.startOfDayValue ?? 100000;
  const todayPnlPercent = startOfDayValue > 0 ? (todayPnl / startOfDayValue) * 100 : 0;

  const weeklyPnl = pnlHistory?.weeklyPnl ?? 0;
  const cumulativePnl = pnlHistory?.cumulativePnl ?? 0;
  const startingBalance = pnlHistory?.startingBalance ?? 100000;
  const weeklyPnlPercent = startingBalance > 0 ? (weeklyPnl / startingBalance) * 100 : 0;
  const cumulativePnlPercent = startingBalance > 0 ? (cumulativePnl / startingBalance) * 100 : 0;

  const currentValue = todayPnlData?.currentValue ?? (user?.balance || 100000);
  ...
  const netWorthData = useMemo(() => {
    ...
    return filteredDailyPnl.map((d) => ({
      date: d.date,
      value: base + d.cumulative,
    }));
  }, [filteredDailyPnl, startingBalance]);
    ...
    <LayoutShell>
      <div className="px-3 py-3 md:p-6 max-w-3xl mx-auto">
        ...
        <Card className="mb-6">
          <CardContent className="p-4">
            <div className="mb-1">
              <span className="text-base font-bold text-foreground">Asset Net Worth</span>
            </div>
            <div className="text-xs text-muted-foreground mb-2">{new Date().toISOString().split("T")[0]}</div>
            <div className={`text-xl font-bold font-mono mb-4 ${pnlColor(periodPnl)}`} data-testid="text-net-worth-value">
              ${currentValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>

            {netWorthData.length > 1 ? (
              <div className="relative h-32" data-testid="chart-net-worth">
        ...
        <Card className="mb-6">
          <CardContent className="p-4">
            <div className="flex items-center justify-between gap-2 mb-1">
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  data-testid="button-daily-chart-view"
                >
                ...
                <Button
                  ...
                  data-testid="button-daily-calendar-view"
                >
            ...
            <div className="text-xs text-muted-foreground mb-2">{new Date().toISOString().split("T")[0]}</div>
            <div className={`text-xl font-bold font-mono mb-4 ${pnlColor(periodPnl)}`} data-testid="text-period-pnl">
              {formatPnl(periodPnl)}
            </div>
            ...
            {dailyViewMode === "calendar" ? (
              <div>
                <div className="flex items-center justify-center gap-4 mb-3">
                  ...
                  <span className="text-sm font-medium text-foreground" data-testid="text-calendar-month">
                    {monthLabel}
                  </span>
                  <Button
                    size="icon"
              ...
              <div className="space-y-1" data-testid="chart-daily-bars">
        ...
        <Card className="mb-6">
          <CardContent className="p-4">
            ...
            <div className="text-xs text-muted-foreground mb-2">{new Date().toISOString().split("T")[0]}</div>
            <div className={`text-xl font-bold font-mono mb-4 ${pnlColor(cumulativePnl)}`} data-testid="text-cumulative-pnl-value">
              {formatPercent(cumulativePnlPercent)}
            </div>

            {filteredDailyPnl.length > 1 ? (
              <div className="relative h-32" data-testid="chart-cumulative-pnl">
                {(() => {
                  const cumulativeData = filteredDailyPnl.map((d) => ({
                    date: d.date,
                    pct: startingBalance > 0 ? (d.cumulative / startingBalance) * 100 : 0,
                  }));
                  const vals = cumulativeData.map((d) => d.pct);
                  ...
                  return (
                    <>
                      ...
                      <svg
                        viewBox={`0 0 ${cumulativeData.length - 1} 100`}
                        className="w-full h-full ml-12"
                        preserveAspectRatio="none"
                      >
                        <path
                          d={cumulativeData
                            .map((d, i) => {
                              ...
                              const y = range > 0 ? 100 - ((d.pct - chartMin) / range) * 100 : 50;
                              return `${i === 0 ? "M" : "L"} ${i} ${y}`;
                            })
                            .join(" ")}
                          fill="none"
        ...
        <Card className="mb-6">
          <CardContent className="p-4">
            ...
            <div className="text-xs text-muted-foreground mb-2">{new Date().toISOString().split("T")[0]}</div>
            <div className={`text-xl font-bold font-mono mb-4 ${pnlColor(periodPnl)}`} data-testid="text-profits-value">
              {formatPnl(periodPnl)}
            </div>

            {profitChartData.length > 0 ? (
              <div data-testid="chart-profits">
